...
public class StudentsTest extends BaseTest {
    ...
    @Test(dataProvider = "searchData", description = "Kiểm tra tính năng tìm kiếm sinh viên với nhiều kịch bản")
    public void testStudentSearch(String keyword, String expectedResult, String scenario) {
        ...
        // 2. Chuyển sang trang Students
        driver.get("https://edupathway.studyguide.dev/admin/students");

        // Khởi tạo Kỹ sư quản lý trang Students
        StudentsPage studentsPage = new StudentsPage(driver);

        // 3. Thực hiện tìm kiếm với từ khóa từ DataProvider
        studentsPage.searchStudent(keyword);

        // 4. Kiểm tra kết quả
        int resultsCount = studentsPage.getNumberOfResults();

        if (expectedResult.equals("FOUND")) {
            Assert.assertTrue(resultsCount > 0,
                    "Kịch bản [" + scenario + "] - Lỗi: Không tìm thấy sinh viên nào với từ khóa: '" + keyword + "'");
    ...
    public Object[][] getSearchData() {
        return new Object[][] {
                // Nhóm Positive
                { "HNG", "FOUND", "TC01 - Tìm kiếm từ khóa chính xác (Exact match)" },
                { "hng", "FOUND", "TC02 - Tìm kiếm không phân biệt hoa thường (Case insensitive)" },
                { "HN", "FOUND", "TC03 - Tìm kiếm một phần của từ (Partial match)" },
                ...
                // Nhóm Edge / Boundary
                { "   HNG   ", "FOUND", "TC07 - Xóa khoảng trắng thừa (Trim input)" },
                { "@#$%^&*()_+", "NO_CRASH", "TC08 - Tìm kiếm với ký tự đặc biệt" },
                { "HNG".repeat(100), "NO_CRASH", "TC09 - Tìm kiếm chuỗi cực kỳ dài (Max length)" },
                ...
                // Nhóm Performance
                { "HNG".repeat(1000), "NO_CRASH", "TC12 - Tìm kiếm với từ khóa cực kỳ dài" },
                { "HNG".repeat(5000), "NO_CRASH", "TC13 - Tìm kiếm với từ khóa siêu dài" }
        };
    }
}
