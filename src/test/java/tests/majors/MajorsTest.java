...
import pages.MajorsPage;

public class MajorsTest extends BaseTest {
    @Test(dataProvider = "searchData", description = "Kiểm tra tính năng tìm kiếm chuyên ngành với nhiều kịch bản")
    public void testMajorsSearch(String keyword, String expectedResult, String scenario) {
        ...
        // 2. Chuyển sang trang Majors
        driver.get("https://edupathway.studyguide.dev/admin/majors");

        // Khởi tạo Kỹ sư quản lý trang Majors
        MajorsPage majorsPage = new MajorsPage(driver);

        // 3. Thực hiện tìm kiếm với từ khóa từ DataProvider
        majorsPage.searchMajor(keyword);

        // 4. Kiểm tra kết quả
        int resultsCount = majorsPage.getNumberOfResults();

        if (expectedResult.equals("FOUND")) {
            Assert.assertTrue(resultsCount > 0,
                    "Kịch bản [" + scenario + "] - Lỗi: Không tìm thấy chuyên ngành nào với từ khóa: '" + keyword
                            + "'");
    ...
    public Object[][] getSearchData() {
        return new Object[][] {
                // Nhóm Positive
                { "business", "FOUND", "TC01 - Tìm kiếm từ khóa chính xác (Exact match)" },
                { "BUSINESS", "FOUND", "TC02 - Tìm kiếm không phân biệt hoa thường (Case insensitive)" },
                { "bus", "FOUND", "TC03 - Tìm kiếm một phần của từ (Partial match)" },
                ...
                // Nhóm Edge / Boundary
                { "   business   ", "FOUND", "TC07 - Xóa khoảng trắng thừa (Trim input)" },
                { "@#$%^&*()_+", "NO_CRASH", "TC08 - Tìm kiếm với ký tự đặc biệt" },
                { "business".repeat(100), "NO_CRASH", "TC09 - Tìm kiếm chuỗi cực kỳ dài (Max length)" },
                ...
                // Nhóm Performance
                { "business".repeat(1000), "NO_CRASH", "TC12 - Tìm kiếm với từ khóa cực kỳ dài" },
                { "business".repeat(5000), "NO_CRASH", "TC13 - Tìm kiếm với từ khóa siêu dài" }
        };
    }
}
